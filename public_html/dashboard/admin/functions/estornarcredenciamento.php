...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $mensAberta = $contratoFinDAO->select('*', "contrato = $credFin->contrato AND status = 0 AND referencia = 'Mensalidade'")->fetchAll(PDO::FETCH_OBJ);
    
    foreach($mensAberta as $mens) {
        $mens->status = '2';
        $contratoFinDAO->updateStatus($mens);
    }
    
    echo json_encode(array(
        'erro' => false,
        'msg' => 'Credenciamento estornado!'
    ), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
}