  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_control_acesso_contrato" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  // ---------- Validate senha
  function do_ajax_control_acesso_contrato_validate_senha()
  {
    var nomeCampo_senha = "senha";
    var var_senha = scAjaxGetFieldText(nomeCampo_senha);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_control_acesso_contrato_validate_senha(var_senha, var_script_case_init, do_ajax_control_acesso_contrato_validate_senha_cb);
  } // do_ajax_control_acesso_contrato_validate_senha

  function do_ajax_control_acesso_contrato_validate_senha_cb(sResp)
  {
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_control_acesso_contrato_validate_senha_cb
  // ---------- Form
  function do_ajax_control_acesso_contrato_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_control_acesso_contrato_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    ...
    scAjaxProcOn();
    x_ajax_control_acesso_contrato_submit_form(var_senha, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_control_acesso_contrato_submit_form_cb);
  } // do_ajax_control_acesso_contrato_submit_form

  function do_ajax_control_acesso_contrato_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
  ...
  } // do_ajax_control_acesso_contrato_submit_form_cb

  var scStatusDetail = {};

  function do_ajax_control_acesso_contrato_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_control_acesso_contrato_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['control_acesso_contrato']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_control_acesso_contrato_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
{
?>
    do_ajax_control_acesso_contrato_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_control_acesso_contrato_navigate_form_cb
  function sc_hide_control_acesso_contrato_form()
  {
    for (var block_id in ajax_block_id) {
      $("#div_" + ajax_block_id[block_id]).hide();
    }
  } // sc_hide_control_acesso_contrato_form
