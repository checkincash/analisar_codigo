<?php
   ...
   $Sel_Groupby = new admcontrato_sel_Groupby(); 
   $Sel_Groupby->Sel_Groupby_init();
   
class admcontrato_sel_Groupby
{
function Sel_Groupby_init()
{
   ...
   $groupby_atual = $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Ind_Groupby'];
...
function Sel_processa_out($sel_groupby)
{
   ...
   {
       if (count($campos_sel) != count($_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp']))
       {
       ...
       {
          $Arr_temp = array();
           foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp'] as $Cada_cmp => $Resto)
           {
               $Arr_temp[] = $Cada_cmp;
           }
   ...
   {
       $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Ind_Groupby']     = $sel_groupby;
       $_SESSION['sc_session'][$sc_init]['admcontrato']['contr_array_resumo'] = "NAO";
       $_SESSION['sc_session'][$sc_init]['admcontrato']['contr_total_geral']  = "NAO";
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['ordem_quebra']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['ordem_select']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['tot_geral']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['arr_total']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_group_by']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_x_axys']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_y_axys']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_fill']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_order']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_order_col']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_order_level']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_order_sort']);
       unset($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_tabular']);
       if ($sel_groupby == "sc_free_group_by")
       {
           $tab_arr_groubby_cmp = array();
           $tab_arr_groubby_sql = array();
           $tab_arr_groubby_cmp['cidade'] = array('cmp' => "cidade", 'ind' => 0, 'format' => "");
           $tab_arr_groubby_cmp['estado'] = array('cmp' => "estado", 'ind' => 1, 'format' => "");
           $tab_arr_groubby_sql[0] = array('cmp' => "cidade", 'ord' => 'asc');
           $tab_arr_groubby_sql[1] = array('cmp' => "estado", 'ord' => 'asc');
           $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp']  = array();
           $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_sql']  = array();
           $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_date_format']['sc_free_group_by'] = array();
           $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_orig'] = array();
           foreach ($campos_sel as $cada_cmp)
           {
               if (!empty($cada_cmp))
               {
                   $ind = $tab_arr_groubby_cmp[$cada_cmp]['ind'];
                   $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp'][$cada_cmp] = $tab_arr_groubby_cmp[$cada_cmp]['cmp'];
                   $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_sql'][$cada_cmp][$tab_arr_groubby_sql[$ind]['cmp']] = $tab_arr_groubby_sql[$ind]['ord'];
                   $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_date_format']['sc_free_group_by'][$cada_cmp] = $tab_arr_groubby_cmp[$cada_cmp]['format'];
               }
           }
       }
   }
   if ($sel_groupby == "sc_free_group_by")
   {
       ...
       $_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_x_axys'] = $xaxys_list;
       $_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_y_axys'] = $yaxys_list;
   }
   if ($opc_ret == "resumo" && !empty($this->summ_fields))
   {
       $summ_fields = explode("@?@", $this->summ_fields);
       if(isset($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'][ $sel_groupby ]))
       {
           foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'][ $sel_groupby ] as $i_sum => $d_sum)
           {
               $_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'][ $sel_groupby ][$i_sum]['display'] = in_array($i_sum, $summ_fields);
           }
       }
       $_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_order'][ $sel_groupby ] = $summ_fields;
       $_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_drill_down'] = $this->drill_down;
   }
?>
...
?>
 <script language="javascript" type="text/javascript">
  function scSubmitGroupBy(sPos) {
   $.ajax({
    type: "POST",
    url: "admcontrato_sel_groupby.php",
    data: {
...
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
...
{
  $max_height = 1;
  if (isset($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
...
$(function() {
 ...
 <tr id="sc_id_free_groupby">
  <td>
<?php
     if (!in_array("sc_free_group_by", $_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Groupby_hide']))
     {
...
<?php
        ...
        foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp'] as $NM_cada_field => $resto)
        {
...
<?php
        ...
        {
           if (!isset($_SESSION['sc_session'][$sc_init]['admcontrato']['SC_Gb_Free_cmp'][$NM_cada_field]))
           {
?>
...
<?php
        if ($bSummaryPage)
        {
            ...
            {
                foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['pivot_x_axys'] as $temp)
                {
                    reset($aTmpGroupBy);
...
?>
 <tr id="sc_id_summary" class="SummaryBox" style='display:<?php echo ($has_total_dynamic)?'':'none';?>'>
  <td>
    ...
    <?php
    foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_control'] as $nome_quebra => $_arr_fields_totals)
    {
    ?>
...
?>
              <li class="sc_ui_litem sc_ui_litem_total scAppDivSelectFieldsEnabled" id="sc_id_item_summ_<?php echo $nome_quebra; ?>_<?php echo $key_total; ?>"><table style="width: 100%"><tr><td><?php echo NM_encode_input($sLabel); ?></td><td style="text-align: right" class="sc-ui-total-options"><?php
...
<?php
          if(isset($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_order'][ $nome_quebra ]) && is_array($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_order'][ $nome_quebra ]) && !empty($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_order'][ $nome_quebra ]))
          {
           foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_order'][ $nome_quebra ] as $i_sum)
           {
            if ('' != $i_sum && isset($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'][ $nome_quebra ][$i_sum]))
            {
                $d_sum = $_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_display'][ $nome_quebra ][$i_sum];
                if ($d_sum['display'])
                {
                    $sLabel = $d_sum['label'];
                    $sId    = '';
                    $bFound = false;
                    $iKey   = $key_total;
                    if(isset($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_control'][$nome_quebra]))
                    {
                        foreach ($_SESSION['sc_session'][$sc_init]['admcontrato']['summarizing_fields_control'][$nome_quebra] as $_key_total => $d_field)
                        {
                            if(is_array($d_field))
                            {
                                    $l_field = $d_field['cmp_res'];
