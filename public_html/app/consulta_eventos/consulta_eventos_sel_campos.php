<?php
   ...
   session_start();
    ...
    $Sel_Cmp = new consulta_eventos_sel_cmp(); 
    $Sel_Cmp->Sel_cmp_init();
   
class consulta_eventos_sel_cmp
{
function Sel_cmp_init()
{
   ...
   $tab_blk_campos[] = "destaque";
   $tab_ger_campos['pk_publicador'] = "on";
   $tab_ger_campos['fk_contrato'] = "on";
   $tab_ger_campos['foto_publicacao'] = "on";
   $tab_ger_campos['texto_publicacao'] = "on";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (isset($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel']) && !empty($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
...
function Sel_processa_out()
{
   ...
   $arr_temp = array();
   if (!isset($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel']))
   {
       $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel'] = array();
   }
   $arr_order = $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order'];
   $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order'] = array();
   $campos_sel = explode("@?@", $this->campos_sel);
   foreach ($campos_sel as $campo_order)
   {
       if (isset($tab_ger_campos[$campo_order]))
       {
           $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order'][] = $campo_order;
       }
   }
   foreach ($arr_order as $campo_order)
   {
       if (!in_array($campo_order, $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order']))
       {
           $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order'][] = $campo_order;
       }
   }
   ...
   $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['usr_cmp_sel'] = $arr_temp;
?>
...
?>
  function scSubmitSelCampos(sPos) {
   ...
   $.ajax({
    type: "POST",
    url: "consulta_eventos_sel_campos.php",
    data: {
...
<?php
   ...
   foreach ($tab_ger_campos as $NM_cada_field => $NM_cada_opc)
   {
       if ($NM_cada_opc != "none")
       {
           if ($NM_cada_opc != "on")
           {
               $prep_combo[strtolower($NM_cada_field)] = $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['labels'][$NM_cada_field];
           }
       }
   }
...
<?php
   if (isset($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order']))
   {
       foreach ($_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['field_order'] as $NM_cada_field)
       {
           if ($tab_ger_campos[$NM_cada_field] == "on")
           {
               ...
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsEnabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['labels'][$NM_cada_field]; ?></li>
<?php
               }
               else
               {
?>
     <li class="sc_ui_litem scAppDivSelectFieldsDisabled" id="sc_id_itemsel_<?php echo NM_encode_input($NM_cada_field); ?>"><?php echo $_SESSION['sc_session'][$this->sc_init]['consulta_eventos']['labels'][$NM_cada_field]; ?></li>
<?php
               }
           }
       }
