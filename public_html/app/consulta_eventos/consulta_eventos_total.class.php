...
class consulta_eventos_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['consulta_eventos']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['consulta_eventos']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (SELECT     pk_publicador,     fk_contrato,     data_criacao,     nomenclatura,     foto_publicacao,     texto_publicacao,     gera_dados,     cep,     bairro,     rua,     numero,     complemento,     cidade,     estado,     latitude,     longitude,     destaque,     pdesconto,     situacao FROM     ap_contrato_publicador where fk_contrato = ( select pk_contrato from ap_contrato where cnpj = '" . $_SESSION['usr_cnpj'] . "' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (SELECT     pk_publicador,     fk_contrato,     data_criacao,     nomenclatura,     foto_publicacao,     texto_publicacao,     gera_dados,     cep,     bairro,     rua,     numero,     complemento,     cidade,     estado,     latitude,     longitude,     destaque,     pdesconto,     situacao FROM     ap_contrato_publicador where fk_contrato = ( select pk_contrato from ap_contrato where cnpj = '" . $_SESSION['usr_cnpj'] . "' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['consulta_eventos']['contr_total_geral'] = "OK";
   } 
