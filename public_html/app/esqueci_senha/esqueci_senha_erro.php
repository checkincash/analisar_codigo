...
$old_error_handler = set_error_handler("esqueci_senha_scriptcase_error_handler");

function esqueci_senha_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['esqueci_senha']['contr_erro']) && $_SESSION['scriptcase']['esqueci_senha']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_esqueci_senha->contr_esqueci_senha->Apl_com_erro = true;
            if ($inicial_esqueci_senha->contr_esqueci_senha->NM_ajax_flag)
            {
                $inicial_esqueci_senha->contr_esqueci_senha->NM_ajax_info['errList']['geral_esqueci_senha'][] = $msg;
                return;
            }
            esqueci_senha_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function esqueci_senha_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['esqueci_senha'])  && '' != $_SESSION['scriptcase']['error_icon']['esqueci_senha'])  ? $_SESSION['scriptcase']['error_icon']['esqueci_senha']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['esqueci_senha']) && '' != $_SESSION['scriptcase']['error_close']['esqueci_senha']) ? $_SESSION['scriptcase']['error_close']['esqueci_senha'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['esqueci_senha']) && '' != $_SESSION['scriptcase']['error_span_title']['esqueci_senha']) ? $_SESSION['scriptcase']['error_span_title']['esqueci_senha'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['esqueci_senha']) && '' != $_SESSION['scriptcase']['error_icon_title']['esqueci_senha']) ? $_SESSION['scriptcase']['error_icon_title']['esqueci_senha'] : '';
?>
