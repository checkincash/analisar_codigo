...
if( $pcategoria != 0 )
{
	...
	$dados_checkin = "SELECT b.pk_publicador as 'id', a.pk_contrato, a.fantasia as 'razao',b.destaque,
	 ...
	 where a.ativo = 1 and b.situacao = 0  and c.pk_classificacao = " . $pcategoria . " HAVING distancia < ". $pdistancia ." 
	 ORDER BY  distancia  
	";
...
{
	$dados_checkin = "SELECT b.pk_publicador as 'id', a.pk_contrato, a.fantasia as 'razao',
	( case when a.classificacao1 is null then '' else a.classificacao1 end) classificacao1, 
	( case when a.classificacao2 is null then '' else a.classificacao2 end) classificacao2, b.latitude, b.longitude,
	 b.destaque, c.descricao as 'categoria', a.cnpj, b.nomenclatura, b.abreviatura, b.rua, b.numero, b.complemento, b.bairro, b.cidade, 
	 b.estado, b.cep, a.telefone, b.pdesconto, a.fachada, b.foto_publicacao, b.texto_publicacao, 
	 b.foto_premiacao, b.texto_premiacao, b.gera_dados, 
	 (case when p.pincash_qtde is null then 0 else p.pincash_qtde end) as pincash,
	 ( select pk_sorteio_cabe_pincash from ap_pincash_sorteio where ativo = 1 ) as idsorteio,
	 (case when a.seg_m_de is not null then substring(a.seg_m_de,1,5) else '00:00' end) seg_m_de,
	 (case when a.seg_m_ate is not null then substring(a.seg_m_ate,1,5) else '00:00' end) seg_m_ate,
